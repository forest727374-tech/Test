-- // KECUYA HUB v3.2 (FULL WORKING FIX FOR MUSCLE LEGENDS)
        ...
        if Flags.AutoBoostEggs then
            pcall(function()
                ...
                if rEvents then
                    ...
                    if openEgg then
                        openEgg:InvokeServer("openEgg", "Crystal Egg")
                        openEgg:InvokeServer("openEgg", "Jungle Egg")
                    end
                end
            end)
        end
...
local function GetTargetRock()
    ...
    local function CheckFolder(folder)
        for _, v in ipairs(folder:GetChildren()) do
            ...
            if cleanName:find(targetName, 1, true) or targetName:find(cleanName, 1, true) then
                if v:IsA("BasePart") then
                    return v
                elseif v:IsA("Model") then
                    return v.PrimaryPart or v:FindFirstChild("Rock") or v:FindFirstChildWhichIsA("BasePart")
                end
            end
        end
        return nil
    ...
    local machines = Workspace:FindFirstChild("machines") or Workspace:FindFirstChild("Machines")
    if machines then
        local found = CheckFolder(machines)
        if found then return found end
    end
    
    return CheckFolder(Workspace)
end
    ...
    while true do
        if Flags.AirRock then
            ...
            if char and rockPart then
                ...
                local topOffset = Vector3.new(0, (rockPart.Size.Y / 2) + 2.5, 0)
                
                hrp.CFrame = CFrame.new(rockPos.Position + topOffset) * CFrame.Angles(math.rad(-90), 0, 0)
                hrp.AssemblyLinearVelocity = Vector3.zero
                
                ExecuteAttack(rockPart)
            end
            task.wait(Flags.FastPunch and 0.005 or 0.03)
        else
            task.wait(0.3)
        end
    end
...
RunService.RenderStepped:Connect(function(delta)
    ...
    if Flags.Fly and FlyBodyVel and FlyBodyGyro then
        ...
        FlyBodyVel.Velocity = (moveDir * FlySpeed) + Vector3.new(0, upDown * FlySpeed, 0)
        FlyBodyGyro.CFrame = cam.CFrame
    end

    if Flags.AntiKnockback then
        for _, v in ipairs(hrp:GetChildren()) do
            if (v:IsA("BodyVelocity") and v ~= FlyBodyVel) or v:IsA("BodyForce") or v:IsA("VectorForce") then
                v:Destroy()
            end
        end
    end
...
-- ==========================================
-- 5. ТАРГЕТНАЯ АТАКА И KARMA FARMING (СТОЯ НА МЕСТЕ)
-- ==========================================
                ...
                if p ~= LocalPlayer and p.Character then
                    ...
                    if not (Flags.IgnoreFriends and isFriend) then
                        ...
                        if tHrp and tHum and tHum.Health > 0 and not tChar:FindFirstChildOfClass("ForceField") then
                            ...
                            -- Атакуем targetPart БЕЗ ТЕЛЕПОРТАЦИИ
                            if isTarget then
                                ExecuteAttack(tHrp)
                            end
                        end
