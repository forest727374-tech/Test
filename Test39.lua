...
-- ==========================================
-- 6. ФАРМ КАМНЕЙ (ИСПРАВЛЕННЫЙ)
-- ==========================================
local function GetRockPart(rockName)
    local nameLower = rockName:lower()
    for _, obj in pairs(Workspace:GetDescendants()) do
        if obj.Name:lower() == nameLower or (obj.Parent and obj.Parent.Name:lower() == nameLower) then
    ...
    while true do
        if Flags.AirRock then
            ...
            if hrp then
                ...
                if rPart then
                    -- Зависаем прямо над камнем и наносим удары
                    hrp.CFrame = rPart.CFrame * CFrame.new(0, 5, 0) * CFrame.Angles(math.rad(-90), 0, 0)
                    ExecuteAttack(rPart)
                end
            end
        end
...
RunService.RenderStepped:Connect(function()
    ...
    if Flags.Fly and hrp and hum then
        ...
        if moveDir.Magnitude > 0 then
            flyVel = (camCF.RightVector * moveDir.X) + (camCF.LookVector * -moveDir.Z)
        end
...
-- ==========================================
-- 8. АТАКА ПО ВОЗДУХУ И ФИЛЬТР КАРМЫ
-- ==========================================
        ...
        if Flags.AutoKillAir or Flags.FarmGoodKarma or Flags.FarmEvilKarma or Flags.TargetKill then
            for _, p in pairs(Players:GetPlayers()) do
                if ValidTarget(p) then
                    local targetKarma = GetKarmaValue(p)
                    local shouldKill = false

                    if Flags.AutoKillAir then
                        shouldKill = true
                    elseif Flags.FarmGoodKarma then
                        -- Убиваем ТОЛЬКО злодеев (Karma < 0)
                        if targetKarma < 0 then shouldKill = true end
                    elseif Flags.FarmEvilKarma then
                        -- Убиваем ТОЛЬКО добряков (Karma >= 0)
                        if targetKarma >= 0 then shouldKill = true end
                    ...
                    if shouldKill then
                        ...
                        if hrp and tHrp then
                            -- Зависаем В ВОЗДУХЕ над противником (на 10 блоков выше)
                            hrp.CFrame = tHrp.CFrame * CFrame.new(0, 10, 0) * CFrame.Angles(math.rad(-90), 0, 0)
                            ExecuteAttack(tHrp)
                        end
                        break
                    end
...
-- ==========================================
CreateToggle(RockPg, "Авто-Фарм Камня (По воздуху)", Flags.AirRock, function(v) Flags.AirRock = v end)
...
CreateToggle(KillPg, "Убивать всех (Auto Kill в воздухе)", Flags.AutoKillAir, function(v) Flags.AutoKillAir = v end)
...
CreateToggle(KillPg, "Убивать выбр. цель (По воздуху)", Flags.TargetKill, function(v) Flags.TargetKill = v end)
