-- // KECUYA HUB (MOBILE & PC OPTIMIZED)
...
-- ИЗОБРАЖЕНИЯ (Загруженный Asset ID)
local MY_ASSET_ID = "132646399120668"
local ANIME_CHARACTER_IMAGE = "rbxassetid://" .. MY_ASSET_ID
local ANIME_BACKGROUND_IMAGE = "rbxassetid://" .. MY_ASSET_ID
...
MainBg.ImageTransparency = 0.2 -- Картинка яркая и четкая
...
-- ==========================================
-- 5. МЕХАНИКА УДАРОВ И ТРЕНИРОВКИ
-- ==========================================
...
local function ExecuteHit(targetPart)
    local cd = Flags.FastPunch and 0.02 or ATTACK_COOLDOWN
...
local function SafeTrain(toolPartialName)
    ...
    for _, t in pairs(char:GetChildren()) do
        if t:IsA("Tool") and t.Name:find(toolPartialName) then tool = t break end
    end
    if not tool then
...
task.spawn(function()
    while true do
        ...
        task.wait(Flags.FastPunch and 0.03 or 0.1)
    end
end)

-- ==========================================
-- 6. ОПТИМИЗИРОВАННЫЙ ФАРМ КАМНЕЙ
-- ==========================================
...
-- ==========================================
-- 7. МОБИЛЬНЫЙ ПОЛЕТ (CAMERA-BASED FLY)
-- ==========================================
local flyBV, flyBG
            ...
            for _, p in pairs(Players:GetPlayers()) do
                if IsValidTarget(p) then
                    ...
                    if Flags.AutoKillAir then 
                        shouldAttack = true
                    elseif Flags.FarmGoodKarma and (evil > 0 or evil > good) then 
                        -- Убиваем Bad игроков, чтобы получить Good Karma
                        shouldAttack = true
                    elseif Flags.FarmEvilKarma and (good > 0 or good >= evil) then 
                        -- Убиваем Good игроков, чтобы получить Evil Karma
                        shouldAttack = true
...
CreateToggle(KillPg, "Фарм Good Karma (Убивать плохих)", Flags.FarmGoodKarma, function(v) Flags.FarmGoodKarma = v end)
CreateToggle(KillPg, "Фарм Evil Karma (Убивать добрых)", Flags.FarmEvilKarma, function(v) Flags.FarmEvilKarma = v end)
