-- // KECUYA HUB (FIXED KILL & KARMA EDITION)
...
ToggleBtn.Text = "⚡"
ToggleBtn.TextColor3 = Color3.fromRGB(192, 132, 252)
...
KeyTitle.Text = "KECUYA HUB  •  @kecuya"
...
KeyInput.Position = UDim2.new(0.075, 0, 0.34, 0)
...
MainTitle.Size = UDim2.new(0, 150, 1, 0)
...
AuthorTag.Position = UDim2.new(0, 92, 0, 0)
...
-- ==========================================
-- 4. ТОЧНЫЙ РАСЧЕТ КАРМЫ И СТАТУСА
-- ==========================================
local function GetKarmaValues(p)
   local goodVal, evilVal = 0, 0
   if not p then return 0, 0 end
   
   -- Прямой поиск по всем возможным местам хранения кармы в игре
   local leaderstats = p:FindFirstChild("leaderstats")
   if leaderstats then
      if leaderstats:FindFirstChild("Good") then goodVal = leaderstats.Good.Value end
      if leaderstats:FindFirstChild("Evil") then evilVal = leaderstats.Evil.Value end
      if leaderstats:FindFirstChild("Good Karma") then goodVal = leaderstats["Good Karma"].Value end
      if leaderstats:FindFirstChild("Evil Karma") then evilVal = leaderstats["Evil Karma"].Value end
   end
   ...
   if p:FindFirstChild("customKarma") then
      local ck = p.customKarma
      if ck:FindFirstChild("Good") then goodVal = ck.Good.Value end
      if ck:FindFirstChild("Evil") then evilVal = ck.Evil.Value end
   end
...
-- ==========================================
-- 5. ИСПРАВЛЕННАЯ СИСТЕМА УДАРОВ (AUTO KILL & KARMA)
-- ==========================================
...
-- ПРАВИЛЬНЫЙ АТАКИРУЮЩИЙ МОДУЛЬ (АВТО КИЛЛ И ФАРМ КАРМЫ)
...
local function AttackTarget(targetPlayer)
   ...
   if not targetChar or not targetChar:FindFirstChild("HumanoidRootPart") or not targetChar:FindFirstChildOfClass("Humanoid") then return end
   if targetChar:FindFirstChildOfClass("Humanoid").Health <= 0 then return end
   ...
   if punch and punch.Parent ~= myChar then
      punch.Parent = myChar
   end

   -- Активация кулака и отправка серверных событий атаки
   if punch then punch:Activate() end
   
   local rEvents = ReplicatedStorage:FindFirstChild("rEvents")
   if rEvents then
      local pRemote = rEvents:FindFirstChild("punchEvent")
      if pRemote then
         pRemote:FireServer("punchAttack")
      end
   end
   
   -- Создаем фейк-клипы хитбокса рядом с врагом
   pcall(function()
      if punch and punch:FindFirstChild("punchHit") then
         punch.punchHit:FireServer(targetChar.HumanoidRootPart)
      end
   end)
end
         ...
         for _, p in pairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and not IsFriend(p) then
               ...
               elseif Flags.FarmGoodKarma and evil > 0 then
                  -- Для хорошей кармы убиваем игроков со злой кармой (Evil > 0)
                  AttackTarget(p)
               elseif Flags.FarmEvilKarma and (good > 0 or evil == 0) then
                  -- Для злой кармы убиваем игроков с хорошей или нейтральной кармой
                  AttackTarget(p)
...
-- АНТИ-ОТБРАСЫВАНИЕ И АВТО-ПОЗИЦИЯ
task.spawn(function()
   while true do
      pcall(function()
         local char = LocalPlayer.Character
...
TargetInput.Size = UDim2.new(1, -4, 0, 24)
