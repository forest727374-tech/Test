-- // KECUYA HUB (SMOOTH NEON EDITION - FIXED FLY)
...
-- ==========================================
-- 2. СОЗДАНИЕ ИНТЕРФЕЙСА С ПЛАВНЫМИ АНИМАЦИЯМИ
-- ==========================================
...
-- Плавная Анимация Появления / Закрытия
...
-- ==========================================
-- 5. ТРЕНИРОВКИ, ФАРМ КАМНЕЙ И ИСПРАВЛЕННЫЙ ПЛАВНЫЙ FLY
-- ==========================================
...
-- ИСПРАВЛЕННЫЙ ИДЕАЛЬНО ПЛАВНЫЙ ПОЛЕТ (БЕЗ ТРЯСКИ И С ПОЛНЫМ ДВИЖЕНИЕМ)
local flyBV, flyBG
...
-- ФУНКЦИОНАЛ АНТИ-ОТБРАСЫВАНИЯ И АВТО-ПОЗИЦИИ
task.spawn(function()
   while true do
      pcall(function()
         local char = LocalPlayer.Character
         if char and char:FindFirstChild("HumanoidRootPart") then
            ...
            if Flags.AntiKnockback then
               for _, v in pairs(hrp:GetChildren()) do
                  if v:IsA("BodyVelocity") and v ~= flyBV then
                     v:Destroy()
                  end
               end
            end
...
-- ==========================================
-- 6. МЕНЮ ВЫБОРА КАМНЯ И НАСТРОЙКИ
-- ==========================================
