-- // KECUYA HUB (HITBOX EXPANDER PVP FIX)
...
local function GetRockPart(rockName)
    ...
    if tick() - LastRockSearchTick < 1.5 then
        return CachedRockPart
    end
    ...
    while true do
        if Flags.AirRock then
            ...
            if hrp then
                ...
                if rPart then
                    hrp.CFrame = rPart.CFrame * CFrame.new(0, 6, 0) * CFrame.Angles(math.rad(-90), 0, 0)
                    ExecuteAttack(rPart)
                end
            end
        end
...
-- Автоматическое расширение хитбоксов и авто-удары без ТП
task.spawn(function()
    while true do
        local activeKill = Flags.AutoKillAir or Flags.FarmGoodKarma or Flags.FarmEvilKarma or Flags.TargetKill
        ...
        for _, p in pairs(Players:GetPlayers()) do
            if ValidTarget(p) then
                ...
                if hrp then
                    if isTarget then
                        -- Увеличиваем хитбокс до твоих рук
                        ...
                        -- Наносим удар стоя на месте
                        GetPunchTool()
                        ExecuteAttack(hrp)
                    else
                        -- Возвращаем исходный хитбокс
