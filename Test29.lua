-- // KECUYA HUB (TARGET SELECTOR EDITION)
...
local function GetRockPart(rockName)
   ...
   for _, obj in pairs(Workspace:GetDescendants()) do
      if obj.Name:lower():find(nameLower) then
...
-- ==========================================
-- 6. КИЛЛ И ВЫБОР ЦЕЛИ (TARGET KILL)
-- ==========================================
...
local function ExecuteAttack(targetPlayer)
   pcall(function()
      ...
      if punch then
         ...
         local pHit = punch:FindFirstChild("punchHit")
         if pHit then pHit:FireServer(tChar.HumanoidRootPart) end
      end
   end)
end
...
-- ИНТЕРАКТИВНЫЙ TARGET SELECTOR (ВЫБОР ИГРОКА)
