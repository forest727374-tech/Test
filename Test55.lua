-- // KECUYA HUB v3.1 (FULL ARCHITECTURE REFACTORED & STABLE)
-- // Author: @kecuya | Key: Guest666
-- // Refactored Target Logic, Dynamic Hitbox Expansion, Unified Position Manager, and Resettable Loops.
...
-- // Аниме Текстуры
local ANIME_GIRL_BG = "rbxassetid://6540342302" 
local ANIME_GIRL_ICON = "rbxassetid://6540342302"
...
local HitboxBoxSize = Vector3.new(6, 6, 6)
...
-- ==========================================
-- 0. БЕЗОПАСНОСТЬ И АВТО-РЕКОННЕКТ (Пункт 9)
-- ==========================================

pcall(function()
    GuiService.ErrorMessageChanged:Connect(function()
        if Flags.AutoReconnect and not IsReconnecting then
            IsReconnecting = true
            task.wait(2)
...
-- ==========================================
-- 2. СИСТЕМА ДИНАМИЧЕСКИХ ХИТБОКСОВ (Пункт 2)
-- ==========================================

local ExtendedHitboxes = {}

local function ClearHitboxes()
    for part, origSize in pairs(ExtendedHitboxes) do
    ...
    table.clear(ExtendedHitboxes)
end

local function ApplyHitbox(targetChar)
    if not targetChar then return end
    local hrp = targetChar:FindFirstChild("HumanoidRootPart")
    if hrp and not ExtendedHitboxes[hrp] then
        ExtendedHitboxes[hrp] = hrp.Size
        pcall(function()
            hrp.Size = HitboxBoxSize
...
-- ==========================================
-- 1. СИСТЕМА АТАКИ И КАСАНИЯ (Пункт 1 & 2)
-- ==========================================
...
local function ExecuteAttack(targetPart)
    local cd = Flags.FastPunch and 0.001 or 0.05
    ...
    if targetPart and targetPart:IsA("BasePart") then
        ...
        if char then
            ...
            if leftHand and typeof(firetouchinterest) == "function" then
                pcall(function()
                    firetouchinterest(targetPart, leftHand, 0)
                    task.wait(0.001)
                    firetouchinterest(targetPart, leftHand, 1)
                end)
            end
            if rightHand and typeof(firetouchinterest) == "function" then
                pcall(function()
                    firetouchinterest(targetPart, rightHand, 0)
                    task.wait(0.001)
                    firetouchinterest(targetPart, rightHand, 1)
                end)
            end
        end
    end
...
-- Проверка валидности врага для атаки
...
-- Единый согласованный цикл PVP/Karma/Target
task.spawn(function()
    while true do
        local anyPvpEnabled = Flags.AutoKillAir or Flags.FarmGoodKarma or Flags.FarmEvilKarma or Flags.TargetKill
        
        if anyPvpEnabled then
            ClearHitboxes()
            for _, p in ipairs(Players:GetPlayers()) do
                if IsValidTarget(p) then
                    ...
                    if Flags.TargetKill then
                        if SelectedTargetPlayer and SelectedTargetPlayer == p then
                            isTarget = true
                        end
                    elseif Flags.FarmGoodKarma then
                        if tKarma < 0 then isTarget = true end
                    elseif Flags.FarmEvilKarma then
                        if tKarma >= 0 then isTarget = true end
                    elseif Flags.AutoKillAir then
                        isTarget = true
                    end
...
-- ==========================================
-- 3. AIR ROCK FARM SYSTEM (Пункт 3)
-- ==========================================

local function GetTargetRock()
    local targetName = SelectedRock:lower():gsub("%s+", "")
    local folder = Workspace:FindFirstChild("machinesFolder") or Workspace
    
    for _, v in ipairs(folder:GetChildren()) do
        local vName = v.Name:lower():gsub("%s+", "")
        if vName:find(targetName, 1, true) or targetName:find(vName, 1, true) then
            if v:IsA("BasePart") then
                return v
            elseif v:IsA("Model") then
                local rockPart = v:FindFirstChild("Rock") or v.PrimaryPart or v:FindFirstChildWhichIsA("BasePart")
                if rockPart then return rockPart end
            end
        end
    end
    return nil
...
task.spawn(function()
    while true do
        if Flags.AirRock then
            ...
            if char and rockPart then
                local hrp = char.HumanoidRootPart
                -- Прямой телепорт в позицию над камнем
                local rockPos = rockPart.CFrame
                hrp.CFrame = CFrame.new(rockPos.Position + Vector3.new(0, (rockPart.Size.Y / 2) + 3, 0))
...
-- ==========================================
-- 4 & 5 & 6. POSITION MANAGER (Fly, AutoPos, AntiKB) (Пункты 4, 5, 6, 10)
-- ==========================================

local function ToggleFlyState(enabled)
    Flags.Fly = enabled
    if MobileFlyFrame then MobileFlyFrame.Visible = enabled end
    ...
    if enabled then
        FlyConnection = RunService.RenderStepped:Connect(function(delta)
            ...
            if not currentChar or not Flags.Fly then return end
            ...
            if hum.MoveVector.Magnitude > 0 and moveDir == Vector3.zero then
                moveDir = (cam.CFrame.RightVector * hum.MoveVector.X) + (cam.CFrame.LookVector * -hum.MoveVector.Z)
            end
            ...
            local finalVelocity = (moveDir * FlySpeed) + Vector3.new(0, upDown * FlySpeed, 0)
            hrp.CFrame = hrp.CFrame + (finalVelocity * delta)
        end)
    end
end

-- ЕДИНЫЙ МЕНЕДЖЕР ФИЗИКИ И ПОЗИЦИОНИРОВАНИЯ (Исключает конфликты)
RunService.RenderStepped:Connect(function()
    ...
    -- Auto Position (Только если не активны Fly и AirRock)
    if Flags.AutoPosition and SavedPosition and not Flags.Fly and not Flags.AirRock then
...
-- Авто-восстановление систем при респавне
...
-- ==========================================
-- 7. ТРЕНИРОВКА И КЛИКЕР (Пункт 7)
-- ==========================================

local function UnifiedTrain(toolPartialName)
    local char = GetCharacter()
    if not char then return end
    ...
    if not tool or not tool.Name:lower():find(toolPartialName:lower(), 1, true) then
        if backpack then
            for _, t in ipairs(backpack:GetChildren()) do
                if t:IsA("Tool") and t.Name:lower():find(toolPartialName:lower(), 1, true) then
    ...
    pcall(function()
        ...
        local rEvents = ReplicatedStorage:FindFirstChild("rEvents")
        if rEvents then
            local repEv = rEvents:FindFirstChild("repEvent") or rEvents:FindFirstChild("trainEvent")
            if repEv then repEv:FireServer() end
        end
    end)
end
...
task.spawn(function()
    while true do
        if Flags.AutoDumbbell then UnifiedTrain("Dumbbell") end
        if Flags.AutoPushups then UnifiedTrain("Pushups") end
        if Flags.AutoSitups then UnifiedTrain("Situps") end
        if Flags.AutoHandstand then UnifiedTrain("Handstand") end
        if Flags.AutoPunch then ExecuteAttack(nil) end
        task.wait(Flags.FastPunch and 0.01 or 0.1)
    end
end)

-- ==========================================
-- 8. AUTO 2X BOOST EGGS (Пункт 8)
-- ==========================================

task.spawn(function()
    while true do
        if Flags.AutoBoostEggs then
            pcall(function()
                local rEvents = ReplicatedStorage:FindFirstChild("rEvents")
                if rEvents then
                    local openEgg = rEvents:FindFirstChild("openEggRemote")
                    if openEgg then openEgg:InvokeServer("openEgg", "2x Boost Egg") end

                    local useBoost = rEvents:FindFirstChild("useBoostRemote") or rEvents:FindFirstChild("boostRemote")
                    if useBoost then useBoost:FireServer("useBoost", "2x Strength") end
                end
            end)
        end
        task.wait(5)
...
-- ==========================================
-- 2. ANTI-AFK & MOBILE CONTROLS (Пункт 10)
-- ==========================================
...
FlyDownBtn.Size = UDim2.new(1, 0, 0, 48)
...
-- ==========================================
-- 7. ANIME GUI И СТИЛИЗАЦИЯ (Полностью Сохранён)
-- ==========================================
...
MainFrame.Size = UDim2.new(0, 390, 0, 260)
MainFrame.Position = UDim2.new(0.5, -195, 0.5, -130)
...
local function AnimateFrame(show)
    if show then
        ...
        TweenService:Create(MainFrame, TweenInfo.new(0.35, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
            Size = UDim2.new(0, 390, 0, 260),
            Position = UDim2.new(0.5, -195, 0.5, -130),
...
local function CreateStatusCard(titleText, accentColor)
    ...
    leftBar.Size = UDim2.new(0, 3, 0, 14)
    leftBar.Position = UDim2.new(0, 6, 0.5, -7)
    ...
    valLbl.Size = UDim2.new(1, -130, 1, 0)
    valLbl.Position = UDim2.new(0, 120, 0, 0)
    valLbl.Text = "..."
...
local DurabilityValLbl = CreateStatusCard("🛡️  Долговечность:", Color3.fromRGB(34, 197, 94))
local KillsValLbl      = CreateStatusCard("⚔️  Убийства (Kills):", Color3.fromRGB(239, 68, 68))
...
task.spawn(function()
    while task.wait(1) do
        ...
        StrengthValLbl.Text = str and FormatNumber(str.Value) or "0"

        local dur = lStats and (lStats:FindFirstChild("Durability") or lStats:FindFirstChild("durability"))
        DurabilityValLbl.Text = dur and FormatNumber(dur.Value) or "0"

        local kl = lStats and (lStats:FindFirstChild("Kills") or lStats:FindFirstChild("kills"))
        KillsValLbl.Text = kl and FormatNumber(kl.Value) or "0"

        local reb = lStats and (lStats:FindFirstChild("Rebirths") or lStats:FindFirstChild("rebirths"))
        RebirthValLbl.Text = reb and FormatNumber(reb.Value) or "0"

        local kv = GetKarmaValue(LocalPlayer)
        KarmaValLbl.Text = FormatNumber(kv)
...
for _, rName in ipairs(RockList) do
...
CreateToggle(BenchPg, "Стойка на руках (Handstand)", Flags.AutoHandstand, function(v) Flags.AutoHandstand = v end)
...
CreateToggle(KillPg, "Игнорировать друзей (Строго)", Flags.IgnoreFriends, function(v) Flags.IgnoreFriends = v end)
...
local function UpdatePlayerList()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p == SelectedTargetPlayer then stillExists = true end
        if p ~= LocalPlayer then
            local isFriend = false
            pcall(function() isFriend = LocalPlayer:IsFriendsWith(p.UserId) end)

            if not (Flags.IgnoreFriends and isFriend) then
    ...
    if not stillExists and SelectedTargetPlayer ~= nil then
...
SizeInput.Size = UDim2.new(1, -4, 0, 24)
SizeInput.PlaceholderText = "Размер персонажа (2)"
SizeInput.Text = "2"
SizeInput.BackgroundColor3 = Color3.fromRGB(18, 18, 28)
SizeInput.BackgroundTransparency = 0.3
SizeInput.TextColor3 = Color3.fromRGB(240, 240, 255)
SizeInput.Font = Enum.Font.Gotham
SizeInput.TextSize = 9
SizeInput.Parent = RebirthPg
Instance.new("UICorner", SizeInput).CornerRadius = UDim.new(0, 6)
SizeInput.FocusLost:Connect(function() AutoSizeValue = tonumber(SizeInput.Text) or 2 end)

CreateToggle(RebirthPg, "Auto Size (Фиксация размера)", Flags.AutoSize, function(v)
    Flags.AutoSize = v
    if v then
        task.spawn(function()
            while Flags.AutoSize do
                pcall(function()
                    local rRemote = ReplicatedStorage:FindFirstChild("rEvents") and ReplicatedStorage.rEvents:FindFirstChild("changeSizeRemote")
                    if rRemote then rRemote:InvokeServer(AutoSizeValue) end
                end)
                task.wait(0.5)
            end
        end)
...
CreateToggle(UtilityPg, "Auto Reconnect (Авто Вход)", Flags.AutoReconnect, function(v) Flags.AutoReconnect = v end)
CreateToggle(UtilityPg, "Auto 2x Boost Eggs", Flags.AutoBoostEggs, function(v) Flags.AutoBoostEggs = v end)
CreateToggle(UtilityPg, "Anti-AFK (+Таймер)", Flags.AntiAFK, function(v) Flags.AntiAFK = v; AntiAfkFrame.Visible = v end)
...
CreateToggle(UtilityPg, "Авто Позиция (Замок)", Flags.AutoPosition, function(v) Flags.AutoPosition = v end)
...
CreateToggle(UtilityPg, "Полет (Fly)", Flags.Fly, function(v) ToggleFlyState(v) end)
