...
RunService.Heartbeat:Connect(function()
    if Flags.AirRock then
        ...
        if rockObj and primaryPart then
            ...
            -- Стабильная регистрация удара: передаем реальную позицию камня
...
-- ==========================================
-- 7. ПОЛЕТ (CROSS-PLATFORM FLY ENGINE: PC + MOBILE)
-- ==========================================
local FlyLinVel = nil
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if Flags.Fly then
        ...
        -- Использование MoveDirection объединяет PC (WASD) и Mobile Joystick в единый вектор относительно камеры
        if hum.MoveDirection.Magnitude > 0 then
            local localMove = camCF:VectorToObjectSpace(hum.MoveDirection)
            moveVector = (camCF.LookVector * -localMove.Z) + (camCF.RightVector * localMove.X)
        end
