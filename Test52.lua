-- // KECUYA HUB (FIXED AUTO-DUMBBELL, FLY PC+MOBILE, AUTO-EGG DISABLED BY DEFAULT)
...
-- ==========================================
-- 0. АВТО-РЕКОННЕКТ И АВТО-БУСТ 2X ЯЙЦА
-- ==========================================
...
local function GetTargetRock()
    ...
    for _, v in pairs(Workspace:GetDescendants()) do
        if v:IsA("BasePart") and (v.Name:lower():gsub("%s+", ""):find(targetName, 1, true)) then
            return v
        end
    end
    return nil
...
-- ==========================================
-- 3. ПОЛНОСТЬЮ ОБНОВЛЕННЫЙ FLY (ПК + ТЕЛЕФОН)
-- ==========================================
RunService.RenderStepped:Connect(function(delta)
    ...
    if hrp and Flags.AntiKnockback then
    ...
    if Flags.AutoPosition and SavedPosition and hrp then
    ...
    if Flags.Fly and hrp and hum then
        ...
        -- Мобильный джойстик (если кнопки ПК не нажаты)
...
-- ==========================================
-- 5. ТРЕНИРОВКА (ОБНОВЛЕННАЯ АВТО-ГАНОТЕЛЬ/ИНСТРУМЕНТЫ)
-- ==========================================
...
CreateToggle(UtilityPg, "Полет (Fly)", Flags.Fly, function(v) Flags.Fly = v; MobileFlyFrame.Visible = v end)
