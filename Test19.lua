-- // KECUYA HUB (ULTRA-COMPACT EDITION)
...
-- Плавная анимация открытия / закрытия окна
...
-- Эстетичный мини-переключатель с фиолетовой подсветкой
      ...
      if Flags.AirRock then
         pcall(function()
            ...
            if char and char:FindFirstChild("HumanoidRootPart") then
               ...
               if targetRock then
                  hrp.CFrame = targetRock.CFrame * CFrame.new(0, targetRock.Size.Y / 2 + 3, 0)
                  ...
                  if pRemote then pRemote:FireServer("punchAttack") end
               end
            end
         end)
      end
...
CreateToggle(KillPg, "Убивать всех", Flags.AutoKillAir, function(v) Flags.AutoKillAir = v end)
CreateToggle(KillPg, "Фарм Доброй Кармы", Flags.FarmGoodKarma, function(v) Flags.FarmGoodKarma = v if v then Flags.FarmEvilKarma = false end end)
CreateToggle(KillPg, "Фарм Злой Кармы", Flags.FarmEvilKarma, function(v) Flags.FarmEvilKarma = v if v then Flags.FarmGoodKarma = false end end)
...
CreateToggle(UtilityPg, "Полет (Fly ПК/Тел)", Flags.Fly, function(v) Flags.Fly = v MobileFlyFrame.Visible = v end)

-- ==========================================
-- 5. АВТОРИЗАЦИЯ И КНОПКИ УПРАВЛЕНИЯ
-- ==========================================
