...
local ANIME_CHARACTER_IMAGE = "rbxassetid://130037433596241"
local ANIME_BACKGROUND_IMAGE = "rbxassetid://130037433596241"
...
local function ExecuteHit(targetPart)
    if tick() - LastAttackTick < (Flags.FastPunch and 0.03 or ATTACK_COOLDOWN) then return end
    LastAttackTick = tick()
    ...
    -- Прямой вызов обработчиков качалки/атаки
...
task.spawn(function()
    while true do
        ...
        task.wait(Flags.FastPunch and 0.04 or 0.12)
    end
end)
        ...
        if Flags.AirRock then
            ...
            if hrp then
                ...
                if rockObj and primaryPart then
                    -- Надежная телепортация и атака по камню
                    hrp.CFrame = primaryPart.CFrame * CFrame.new(0, 0, 3)
...
-- ==========================================
-- 7. ПОЛЕТ (CROSS-PLATFORM FLY ENGINE - BODYVELOCITY)
-- ==========================================
local flyBV, flyBG
...
RunService.RenderStepped:Connect(function()
    ...
    if Flags.Fly then
        ...
        if hum and hum.MoveVector.Magnitude > 0 then
            local raw = hum.MoveVector
            moveDir = (cam.CFrame.RightVector * raw.X) + (cam.CFrame.LookVector * (-raw.Z))
        end
        ...
        local finalVel = moveDir + Vector3.new(0, upDown, 0)
        if finalVel.Magnitude > 0 then
            flyBV.Velocity = finalVel.Unit * FlySpeed
        else
            flyBV.Velocity = Vector3.zero
        end
        ...
        if flyBV then CleanFlyPhysics() end
    end
end)

-- ==========================================
-- 8. ИСПОЛНЕНИЕ PVP-АТАКИ (ОПТИМИЗИРОВАННАЯ)
-- ==========================================
task.spawn(function()
    while true do
        if Flags.AutoKillAir or Flags.FarmGoodKarma or Flags.FarmEvilKarma or Flags.TargetKill then
            for _, p in pairs(Players:GetPlayers()) do
                if IsValidTarget(p) then
                    ...
                    if Flags.AutoKillAir then shouldAttack = true
                    elseif Flags.FarmGoodKarma and evil > 0 then shouldAttack = true
                    elseif Flags.FarmEvilKarma and (good > 0 or evil == 0) then shouldAttack = true
                    elseif Flags.TargetKill and SelectedTargetPlayer == p then shouldAttack = true end

                    if shouldAttack then
                        local char = LocalPlayer.Character
                        if char and char:FindFirstChild("HumanoidRootPart") then
                            char.HumanoidRootPart.CFrame = targetHrp.CFrame * CFrame.new(0, 0, 2)
