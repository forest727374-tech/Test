-- // KECUYA HUB (ANIME FIXED EDITION)
...
-- Точные имена камней Muscle Legends
local RockList = {
   "Medium Rock",
   "Large Rock", 
   "Huge Rock", 
...
-- Улучшенный поиск объектов камней на карте
local function GetRockPart(rockName)
   local searchTarget = rockName:lower()
   for _, obj in pairs(Workspace:GetDescendants()) do
      local objName = obj.Name:lower()
      if objName:find(searchTarget) or (obj.Parent and obj.Parent.Name:lower():find(searchTarget)) then
      ...
      pcall(function()
         ...
         if char and char:FindFirstChild("HumanoidRootPart") then
            ...
            if rockPart then
               ...
               rockPart.CFrame = hrp.CFrame * CFrame.new(0, 0, -3)
...
-- ПОЛНЫЙ ФИКС ПОЛЕТА (без просадок вниз и свободное направление)
...
-- Точный удар с перемещением и прямой отправкой Remote
local function ExecuteAttack(targetPlayer)
   pcall(function()
      ...
      -- Дополнительный эвент урона
      if rEvents and rEvents:FindFirstChild("damageEvent") then
         rEvents.damageEvent:FireServer(tChar.HumanoidRootPart)
      end
   end)
end
