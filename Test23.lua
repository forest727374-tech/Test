-- // KECUYA HUB (FLY & ROCKS OVERHAUL EDITION)
...
local function GetKarmaValues(p)
   ...
   local gObj = (p:FindFirstChild("leaderstats") and p.leaderstats:FindFirstChild("Good")) or p:FindFirstChild("Good") or p:FindFirstChild("goodKarma")
   if gObj then goodVal = gObj.Value end
   local eObj = (p:FindFirstChild("leaderstats") and p.leaderstats:FindFirstChild("Evil")) or p:FindFirstChild("Evil") or p:FindFirstChild("evilKarma")
   if eObj then evilVal = eObj.Value end
...
-- ==========================================
-- 5. МЕХАНИКИ: ТРЕНИРОВКИ, ФАРМ КАМНЕЙ, FLY
-- ==========================================
...
-- ХИТБОКС КАМНЕЙ К ИГРОКУ (ФАРМ ПО ВОЗДУХУ)
   ...
   for _, obj in pairs(Workspace:GetDescendants()) do
      if obj:IsA("BasePart") or obj:IsA("Model") then
         ...
         if n:find(nameLower) or (obj.Parent and obj.Parent.Name:lower():find(nameLower)) then
            ...
            elseif obj:IsA("Model") and obj.PrimaryPart then return obj.PrimaryPart
            elseif obj:IsA("Model") and obj:FindFirstChildWhichIsA("BasePart") then return obj:FindFirstChildWhichIsA("BasePart") end
         end
      end
   end
   return nil
      ...
      if Flags.AirRock then
         pcall(function()
            ...
            if char and char:FindFirstChild("HumanoidRootPart") then
               ...
               if rockPart then
                  -- Телепортируем Хитбокс/Парт камня прямо к игроку (по воздуху перед лицом)
                  rockPart.CFrame = hrp.CFrame * CFrame.new(0, 0, -2.5)
...
-- ПВП ПО ВОЗДУХУ (БЕЗ ТП К ИГРОКАМ)
   ...
   while true do
      pcall(function()
         if Flags.AutoKillAir then
            for _, p in pairs(Players:GetPlayers()) do if p ~= LocalPlayer and not IsFriend(p) then AirHitPlayer(p) end end
     elseif Flags.FarmGoodKarma then
            for _, p in pairs(Players:GetPlayers()) do
               if p ~= LocalPlayer and not IsFriend(p) then
                  local _, evil = GetKarmaValues(p)
                  if evil > 0 then AirHitPlayer(p) end
               end
            end
         elseif Flags.FarmEvilKarma then
            for _, p in pairs(Players:GetPlayers()) do
               if p ~= LocalPlayer and not IsFriend(p) then
                  local good, _ = GetKarmaValues(p)
                  if good > 0 then AirHitPlayer(p) end
               end
            end
...
-- ПОДВИЖНЫЙ И СВОБОДНЫЙ FLY (УПРАВЛЕНИЕ КАМЕРОЙ)
   ...
   while true do
      pcall(function()
         ...
         if Flags.Fly and char and char:FindFirstChild("HumanoidRootPart") then
            ...
            -- Свободное вращение персонажа за камерой
            flyBodyGyro.CFrame = camera.CFrame
...
DropdownBtn.Text = "  Камень: " .. SelectedRock .. " ▼"
DropdownBtn.TextColor3 = Color3.fromRGB(168, 85, 247)
...
DropdownList.Size = UDim2.new(1, 0, 0, #RockList * 20)
...
DropdownBtn.MouseButton1Click:Connect(function()
   ...
   local targetSize = isExpanded and UDim2.new(1, -4, 0, 26 + (#RockList * 22)) or UDim2.new(1, -4, 0, 26)
   TweenService:Create(DropdownFrame, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = targetSize}):Play()
end)

for _, rName in pairs(RockList) do
   local itemBtn = Instance.new("TextButton")
   itemBtn.Size = UDim2.new(1, -6, 0, 20)
   itemBtn.Position = UDim2.new(0, 3, 0, 0)
   itemBtn.Text = "   " .. rName
   ...
   itemBtn.MouseButton1Click:Connect(function()
      ...
      DropdownBtn.Text = "  Камень: " .. SelectedRock .. " ▼"
      isExpanded = false
      TweenService:Create(DropdownFrame, TweenInfo.new(0.2), {Size = UDim2.new(1, -4, 0, 26)}):Play()
   end)
end
...
TargetInput.PlaceholderText = "Ник цели..."
TargetInput.Text = ""
...
TargetInput.FocusLost:Connect(function() TargetPlayerName = TargetInput.Text end)
