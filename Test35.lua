...
RunService.Heartbeat:Connect(function()
    if Flags.AirRock then
        ...
        if rockObj and primaryPart then
            ...
            -- Валидация позиционирования: персонаж телепортируется к серверным координатам камня для гарантированного засчета попаданий
            local targetCFrame = ActiveRockData.OriginalCFrame * CFrame.new(0, 0, 3.5)
            hrp.CFrame = targetCFrame

            LocalHitboxPart.CFrame = ActiveRockData.OriginalCFrame
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if Flags.Fly then
        ...
        -- Универсальный сбор вектора движения (ПК джойстик + Мобильный сенсор)
        local moveVector = Vector3.zero
        if hum then
            local rawMove = hum.MoveVector
            if rawMove.Magnitude > 0 then
                moveVector = (cam.CFrame.RightVector * rawMove.X) + (cam.CFrame.LookVector * (-rawMove.Z))
            end
        end

        -- Дополнительный вертикальный вектор (Space/Shift/Mobile Up/Down)
        local upDown = 0
        ...
        local finalVector = moveVector + Vector3.new(0, upDown, 0)

        if finalVector.Magnitude > 0 then
            FlyLinVel.VectorVelocity = finalVector.Unit * FlySpeed
        else
            FlyLinVel.VectorVelocity = Vector3.zero
        end
