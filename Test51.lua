-- // KECUYA HUB (FIXED LAUNCH / ZERO DELETIONS)
...
local function GetTargetRock()
    ...
    for _, obj in pairs(Workspace:GetDescendants()) do
        if obj:IsA("BasePart") or obj:IsA("Model") then
            ...
            if nameClean:find(selectedClean, 1, true) or selectedClean:find(nameClean, 1, true) then
        ...
        if myHrp then
            for _, p in pairs(Players:GetPlayers()) do
                if p ~= LocalPlayer and p.Character then
                    ...
                    if Flags.IgnoreFriends and isFriend then 
                        continue 
                    end
...
-- ИНИЦИАЛИЗАЦИЯ КОНТЕЙНЕРА И ВЫПАДАЮЩЕГО СПИСКА ИГРОКОВ (ИСПРАВЛЕНО)
