-- // KECUYA HUB (FULL FIXED & EXPANDED EDITION)
...
-- ==========================================
-- 4. ВКЛАДКА «СТАТУС» И ВСПОМОГАТЕЛЬНЫЕ ПОИСКИ
-- ==========================================
...
-- БЕЗ-ТЕЛЕПОРТАЦИОННЫЙ (ПО ВОЗДУХУ) КИЛЛ И КАРМА
...
local function AirHitPlayer(p)
   pcall(function()
      ...
      -- Удар отправляется напрямую по воздуху (без физического ТП персонажа к игроку)
      ...
      if r then 
         -- Передача аргументов атаки по серверу
         r:FireServer("punchAttack")
      end
   end)
end
      ...
      pcall(function()
         if Flags.AutoKillAir then
            for _, p in pairs(Players:GetPlayers()) do
               if p ~= LocalPlayer and not IsFriend(p) then AirHitPlayer(p) end
            end
         elseif Flags.FarmGoodKarma then
            -- Хорошая карма: бить тех у кого есть Зло (Evil > 0)
            for _, p in pairs(Players:GetPlayers()) do
         ...
         elseif Flags.FarmEvilKarma then
            -- Плохая карма: бить тех у кого есть Добро (Good > 0)
            for _, p in pairs(Players:GetPlayers()) do
               if p ~= LocalPlayer and not IsFriend(p) then
...
-- ИСПРАВЛЕННЫЙ ПОЛЕТ (БЕЗ ВРАЩЕНИЯ И КРУЧЕНИЯ)
local flyAttachment, flyLinearVelocity, flyAngularVelocity
task.spawn(function()
   while true do
      pcall(function()
         local char = LocalPlayer.Character
         if Flags.Fly and char and char:FindFirstChild("HumanoidRootPart") then
            ...
            if not flyAttachment or flyAttachment.Parent ~= hrp then
               ...
               -- Блокировка кручения/вращения персонажа
               flyAngularVelocity = Instance.new("AngularVelocity")
               flyAngularVelocity.MaxTorque = 9e9
