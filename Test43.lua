-- // KECUYA HUB (FULL COMPLETE CODE - NOTHING DELETED)
...
-- ==========================================
-- 3. ПОИСК КАМНЕЙ И ИХ ФАРМ
-- ==========================================
local function FindRock()
    TargetRockInstance = nil
    local targetName = SelectedRock:lower()
    ...
    for _, obj in pairs(Workspace:GetDescendants()) do
        ...
        local isMatch = false
        if targetName:find("tiny") and oName:find("tiny") then isMatch = true
        elseif targetName:find("medium") and oName:find("medium") then isMatch = true
        elseif targetName:find("large") and oName:find("large") then isMatch = true
        elseif targetName:find("huge") and oName:find("huge") then isMatch = true
        elseif targetName:find("legends") and oName:find("legends") then isMatch = true
        elseif targetName:find("jungle") and oName:find("jungle") then isMatch = true
        elseif targetName:find("king") and oName:find("king") then isMatch = true
        end
    ...
    while true do
        if Flags.AirRock then
            ...
            if hrp and TargetRockInstance then
                hrp.CFrame = TargetRockInstance.CFrame * CFrame.new(0, TargetRockInstance.Size.Y / 2 + 3, 0)
...
RunService.RenderStepped:Connect(function()
    ...
    if Flags.Fly and hrp and hum then
        ...
        if moveDir.Magnitude > 0 then
            flyVector = (cam.CFrame.RightVector * moveDir.X) + (cam.CFrame.LookVector * -moveDir.Z)
        end
        ...
        flyVector = flyVector + Vector3.new(0, upDown, 0)
...
-- ==========================================
-- 5. АВТО-УБИЙСТВО И МАГНИТ ХИТБОКСОВ
-- ==========================================
task.spawn(function()
    while true do
            ...
            for _, p in pairs(Players:GetPlayers()) do
                if p ~= LocalPlayer and p.Character then
                    ...
                    if tHrp and tHum and tHum.Health > 0 and not tChar:FindFirstChildOfClass("ForceField") then
                        ...
                        if Flags.AutoKillAir then isTarget = true
                        elseif Flags.FarmGoodKarma and tKarma < 0 then isTarget = true
                        elseif Flags.FarmEvilKarma and tKarma >= 0 then isTarget = true
                        elseif Flags.TargetKill and SelectedTargetPlayer == p then isTarget = true
                        end

                        if isTarget then
                            tHrp.Size = Vector3.new(Flags.HitboxSize, Flags.HitboxSize, Flags.HitboxSize)
                            ...
                            if tHrp.Size ~= Vector3.new(2, 2, 1) then
...
-- ==========================================
-- РАДУЖНОЕ КРАСИВОЕ НАЗВАНИЕ В МЕНЮ
-- ==========================================
...
-- Динамический радужный градиент
...
-- Анимация вращения/перелива радуги
task.spawn(function()
    while true do
...
CreateToggle(KillPg, "Убивать всех (Магнит хитбоксов)", Flags.AutoKillAir, function(v) Flags.AutoKillAir = v end)
CreateToggle(KillPg, "Фарм Good Karma (Магнит Evil)", Flags.FarmGoodKarma, function(v) Flags.FarmGoodKarma = v end)
CreateToggle(KillPg, "Фарм Evil Karma (Магнит Good)", Flags.FarmEvilKarma, function(v) Flags.FarmEvilKarma = v end)
...
CreateToggle(KillPg, "Убивать выбр. цель (Магнит)", Flags.TargetKill, function(v) Flags.TargetKill = v end)

-- ==========================================
-- РЕБИРТ (С АВТО-РАЗМЕРОМ И БЕСКОНЕЧНЫМ РЕБИРТОМ)
-- ==========================================
RebirthPg.AutomaticCanvasSize = Enum.AutomaticSize.Y

-- 1. БЕСКОНЕЧНЫЙ РЕБИРТ
