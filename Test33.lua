-- // KECUYA HUB (TECHNICAL AUDIT & FUNCTIONAL FIX)
...
local ANIME_CHARACTER_IMAGE = "rbxassetid://6071575925"
local ANIME_BACKGROUND_IMAGE = "rbxassetid://6071579140"
...
-- ==========================================
-- 1. ГЛОБАЛЬНОЕ СОСТОЯНИЕ И ФЛАГИ
-- ==========================================
...
-- Контроль задержки ударов (Anti-Spam Filter)
...
-- Хранилище исходного состояния камня
local ActiveRockData = {
    ModelOrPart = nil,
    PrimaryPart = nil,
    OriginalCFrame = nil,
...
-- Создание специальной локальной зоны коллизии для фарма
local LocalHitboxPart = Instance.new("Part")
LocalHitboxPart.Name = "Kecuya_LocalHitbox"
LocalHitboxPart.Size = Vector3.new(10, 10, 10)
LocalHitboxPart.Transparency = 0.8
LocalHitboxPart.Color = Color3.fromRGB(168, 85, 247)
LocalHitboxPart.Material = Enum.Material.ForceField
LocalHitboxPart.CanCollide = false
LocalHitboxPart.Anchored = true
LocalHitboxPart.Parent = nil

-- ==========================================
-- 2. СИСТЕМА ИНТЕРФЕЙСА (UI)
-- ==========================================
...
-- ==========================================
-- 5. МОДУЛЬ АТАКИ И БАТЛ-МЕХАНИКА (ATTACK ENGINE)
-- ==========================================
local function GetPunchTool()
    local char = LocalPlayer.Character
    if not char then return nil end
    return char:FindFirstChild("Punch") or LocalPlayer.Backpack:FindFirstChild("Punch")
end

-- Модуль 1: Поиск инструмента
...
-- Модуль 2: Валидация сущности для атаки
local function IsValidTarget(p)
    ...
    -- Проверка на щит спавна (ForceField)
...
-- Модуль 3: Обработка и отправка удара
local function ExecuteHit(targetPart)
    if tick() - LastAttackTick < (Flags.FastPunch and 0.05 or ATTACK_COOLDOWN) then return end
    LastAttackTick = tick()

    local punchTool = EnsurePunchEquipped()
    if not punchTool then
        warn("[KECUYA HUB]: Инструмент Punch не найден в инвентаре.")
        return
    end

    -- Активация локальной анимации
    punchTool:Activate()

    -- Вызов сетевого события атаки
    ...
    if rEvents then
        ...
        if targetPart and targetPart:IsA("BasePart") then
            -- Вызов точечного урона по объекту/игроку
            local damageEv = rEvents:FindFirstChild("damageEvent")
            if damageEv then
                damageEv:FireServer(targetPart)
            end
            
            local punchHit = punchTool:FindFirstChild("punchHit")
            if punchHit then
                punchHit:FireServer(targetPart)
            end
        end
    else
        warn("[KECUYA HUB]: ReplicatedStorage.rEvents отсутствует.")
    end
end
...
task.spawn(function()
    while true do
        ...
        if Flags.AutoPunch then ExecuteHit(nil) end
        task.wait(Flags.FastPunch and 0.05 or 0.15)
    end
end)

-- ==========================================
-- 6. ОБРАБОТЧИК КАМНЕЙ (STABLE ROCK FARM)
-- ==========================================
local function RestoreOriginalRockState()
    if ActiveRockData.ModelOrPart and ActiveRockData.ModelOrPart.Parent then
        if ActiveRockData.ModelOrPart:IsA("Model") then
            ActiveRockData.ModelOrPart:PivotTo(ActiveRockData.OriginalCFrame)
        elseif ActiveRockData.ModelOrPart:IsA("BasePart") then
            ActiveRockData.ModelOrPart.CFrame = ActiveRockData.OriginalCFrame
        end

        if ActiveRockData.PrimaryPart then
            ActiveRockData.PrimaryPart.CanCollide = ActiveRockData.OriginalCanCollide
            ActiveRockData.PrimaryPart.Anchored = ActiveRockData.OriginalAnchored
        end
    end

    ActiveRockData.ModelOrPart = nil
    ActiveRockData.PrimaryPart = nil
    ActiveRockData.OriginalCFrame = nil
    LocalHitboxPart.Parent = nil
end

local function FindRockObject(rockName)
    local search = rockName:lower()
    for _, obj in pairs(Workspace:GetDescendants()) do
        if obj.Name:lower() == search or (obj.Parent and obj.Parent.Name:lower() == search) then
            if obj:IsA("Model") then
                local pPart = obj.PrimaryPart or obj:FindFirstChildWhichIsA("BasePart")
                if pPart then return obj, pPart end
...
RunService.Heartbeat:Connect(function()
    if Flags.AirRock then
        ...
        local rockObj, primaryPart = FindRockObject(SelectedRock)
        if rockObj and primaryPart then
            -- Если камень сменился, восстанавливаем прошлый
            if ActiveRockData.ModelOrPart ~= rockObj then
                RestoreOriginalRockState()
                ActiveRockData.ModelOrPart = rockObj
                ActiveRockData.PrimaryPart = primaryPart
                ActiveRockData.OriginalCFrame = rockObj:IsA("Model") and rockObj:GetPivot() or primaryPart.CFrame
                ActiveRockData.OriginalCanCollide = primaryPart.CanCollide
                ActiveRockData.OriginalAnchored = primaryPart.Anchored
            end
            ...
            if rockObj:IsA("Model") then
                rockObj:PivotTo(targetCFrame)
            else
                primaryPart.CFrame = targetCFrame
            end

            -- Подключаем визуально-стабильный хитбокс
            LocalHitboxPart.CFrame = targetCFrame
            LocalHitboxPart.Parent = Workspace

            ExecuteHit(primaryPart)
        end
    else
        if ActiveRockData.ModelOrPart then
            RestoreOriginalRockState()
        end
    end
end)
...
-- ==========================================
-- 7. ПОЛЕТ (PHYSICS-BASED FLY ENGINE)
-- ==========================================
local FlyLinVel = nil
local FlyAlignRot = nil
local FlyAttach = nil

local function CleanFlyPhysics()
    if FlyLinVel then FlyLinVel:Destroy() FlyLinVel = nil end
    if FlyAlignRot then FlyAlignRot:Destroy() FlyAlignRot = nil end
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if not char or not char:FindFirstChild("HumanoidRootPart") then 
        if Flags.Fly then CleanFlyPhysics() end
        return 
    end
    ...
    if Flags.Fly then
        ...
        if not FlyAttach or FlyAttach.Parent ~= hrp then
            CleanFlyPhysics()
            
            FlyAttach = Instance.new("Attachment")
            FlyAttach.Name = "KecuyaFlyAttachment"
            FlyAttach.Parent = hrp

            FlyLinVel = Instance.new("LinearVelocity")
            FlyLinVel.Attachment0 = FlyAttach
            FlyLinVel.MaxForce = 999999
            FlyLinVel.VectorVelocity = Vector3.zero
            FlyLinVel.RelativeTo = Enum.ActuatorRelativeTo.World
            FlyLinVel.Parent = hrp

            FlyAlignRot = Instance.new("AlignOrientation")
            FlyAlignRot.Attachment0 = FlyAttach
            FlyAlignRot.Mode = Enum.OrientationAlignmentMode.OneAttachment
            FlyAlignRot.Responsiveness = 200
            FlyAlignRot.MaxTorque = 999999
            FlyAlignRot.Parent = hrp
        end
        ...
        if UserInputService:IsKeyDown(Enum.KeyCode.Space) or MobileUp then moveVector = moveVector + Vector3.new(0, 1, 0) end
        if UserInputService:IsKeyDown(Enum.KeyCode.LeftShift) or MobileDown then moveVector = moveVector - Vector3.new(0, 1, 0) end

        if moveVector.Magnitude > 0 then
            FlyLinVel.VectorVelocity = moveVector.Unit * FlySpeed
        else
            FlyLinVel.VectorVelocity = Vector3.zero
        end

        FlyAlignRot.CFrame = cam.CFrame
    else
        if FlyAttach then
            CleanFlyPhysics()
        end
...
-- ==========================================
-- 8. ИСПОЛНЕНИЕ PVP-АТАКИ
-- ==========================================
task.spawn(function()
    while true do
        if Flags.AutoKillAir or Flags.FarmGoodKarma or Flags.FarmEvilKarma or Flags.TargetKill then
        ...
        if char and char:FindFirstChild("HumanoidRootPart") then
            ...
            if Flags.AntiKnockback and not Flags.Fly then
                for _, obj in pairs(hrp:GetChildren()) do
                    if (obj:IsA("BodyVelocity") or obj:IsA("BodyForce") or obj:IsA("VectorForce")) and obj ~= FlyLinVel then
                        obj:Destroy()
                    end
                end
            end
...
-- ==========================================
-- 9. ИНТЕРФЕЙСНЫЕ ВКЛАДКИ И СПИСКИ
-- ==========================================
CreateToggle(RockPg, "Авто-Фарм Камня", Flags.AirRock, function(v) 
    Flags.AirRock = v 
    if not v then RestoreOriginalRockState() end
end)
...
CreateToggle(UtilityPg, "Полет (Fly)", Flags.Fly, function(v) 
    ...
    if not v then CleanFlyPhysics() end
end)

-- ==========================================
-- 10. АВТОРИЗАЦИЯ И ИНИЦИАЛИЗАЦИЯ (RESPAWN SAFE)
-- ==========================================
...
-- Гарантированное сбрасывание физических связей при смерти/респавне
