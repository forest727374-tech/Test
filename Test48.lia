-- // KECUYA HUB (FULL FIX: ROCKS, FRIEND CHECK, AUTO-TARGET REFRESH, FLY, AUTO-POSITION, EXPANDED STATS, RECONNECT & BOOST EGGS)
...
-- Рабочие Аниме Текстуры
local ANIME_GIRL_BG = "rbxassetid://6071575925" 
local ANIME_GIRL_ICON = "rbxassetid://6071575925"
...
-- Авто-открытие х2 буст яиц каждые 30 минут
task.spawn(function()
    while true do
        if Flags.AutoBoostEggs then
            ...
            if rEvents then
                ...
                if openEgg then
                    -- Запрос на открытие яиц с бустами
                    pcall(function()
        ...
        task.wait(1800) -- 30 минут (1800 сек)
    end
end)
...
-- ==========================================
-- 2. ПОИСК И ФАРМ КАМНЕЙ (ТОЧНОЕ ТП И БЕЗ СНОСА)
-- ==========================================
    ...
    while true do
        if Flags.AirRock then
            ...
            if hrp and rockPart then
                hrp.CFrame = rockPart.CFrame * CFrame.new(0, (rockPart.Size.Y / 2) + 2.5, 0)
...
-- ==========================================
-- 3. ПОЛЕТ (FLY) И ЖЕСТКАЯ АВТО-ПОЗИЦИЯ
-- ==========================================
...
-- ==========================================
-- 4. АВТО-УБИЙСТВА С СТРОГОЙ ПРОВЕРКОЙ ДРУЗЕЙ
-- ==========================================
task.spawn(function()
    while true do
        ...
        if myHrp then
            for _, p in pairs(Players:GetPlayers()) do
                if p ~= LocalPlayer and p.Character then
                    -- ЖЕСТКИЙ ИГНОР ДРУЗЕЙ
                    if Flags.IgnoreFriends and LocalPlayer:IsFriendsWith(p.UserId) then 
                        continue 
                    end
...
-- ==========================================
-- 7. РАСШИРЕННАЯ СТАТИСТИКА (ХАРАКТЕРИСТИКИ)
-- ==========================================
...
-- ==========================================
-- 8. УПРАВЛЕНИЕ КАМНЯМИ, ТАРГЕТОМ И УТИЛИТАМИ
-- ==========================================
...
-- АВТО-ОБНОВЛЯЕМЫЙ СПИСОК ИГРОКОВ В ТАРГЕТЕ
...
local function UpdatePlayerList()
    ...
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and not (Flags.IgnoreFriends and LocalPlayer:IsFriendsWith(p.UserId)) then
...
CreateToggle(UtilityPg, "Auto 2x Boost Eggs (30 мин)", Flags.AutoBoostEggs, function(v) Flags.AutoBoostEggs = v end)
