-- // KECUYA HUB (FULL FIX: ROCKS, KARMA, ANIME THEME, FORMATTED STRENGTH, FLY & DUMBBELLS)
...
-- Валидные рабочие текстуры аниме
local ANIME_GIRL_BG = "rbxassetid://13861058204"
local ANIME_GIRL_ICON = "rbxassetid://14207936166"
...
-- ==========================================
-- ВПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (ФОРМАТИРОВАНИЕ И АТАКА)
-- ==========================================

-- Форматирование чисел в буквы (K, M, B, T...)
...
-- ==========================================
-- 2. ПОИСК И ФАРМ КАМНЕЙ (ТОЧНОЕ ТП)
-- ==========================================
    ...
    while true do
        if Flags.AirRock then
            ...
            if hrp and rockPart then
                hrp.CFrame = rockPart.CFrame * CFrame.new(0, (rockPart.Size.Y / 2) + 2, 0)
...
-- ==========================================
-- 4. ИСПРАВЛЕННАЯ КАРМА И ЗАЩИТА ОТ ВЫКИДЫВАНИЯ
-- ==========================================
task.spawn(function()
    while true do
                ...
                if p ~= LocalPlayer and p.Character then
                    ...
                    if tHrp and tHum and tHum.Health > 0 and not tChar:FindFirstChildOfClass("ForceField") then
                        ...
                        elseif Flags.FarmGoodKarma and tKarma < 0 then 
                            -- Хорошая карма = убиваем Evil (Karma < 0)
                            isTarget = true
                        elseif Flags.FarmEvilKarma and tKarma >= 0 then 
                            -- Плохая карма = убиваем Good (Karma >= 0)
                            isTarget = true
                        ...
                        if isTarget then
                            -- Фиксируем позицию врага пред кулаками, гася его импульс
...
-- ==========================================
-- 5. АВТО-ПОЗИЦИЯ И ANTI-KNOCKBACK
-- ==========================================
...
-- ==========================================
-- 8. ВЛАДКА СТАТУСА С БУКВЕННОЙ СИЛОЙ
-- ==========================================
