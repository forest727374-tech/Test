-- // MUSCLE LEGENDS ULTIMATE HUB (FIXED AUTO KILL EDITION)
-- // Key: Guest666
...
-- ==========================================
-- 3. ИСПРАВЛЕННЫЙ AUTO KILL И ПАКЕТЫ
-- ==========================================

-- Функция проведения атаки с принудительной отправкой сетевого события Muscle Legends
local function HitTarget(targetPlayer)
   pcall(function()
      if not targetPlayer or targetPlayer == LocalPlayer or Whitelist[targetPlayer.Name] then return end
...
-- Цикл атаки по всем игрокам (Auto Kill All)
task.spawn(function()
   while true do
...
-- Цикл атаки по выбранному цели (Target Kill)
task.spawn(function()
   while true do
               ...
               if not currentRockPart or not currentRockPart.Parent then
                  for _, v in pairs(Workspace:GetChildren()) do
                     if v.Name:lower():find(SelectedRock:lower()) or (v:FindFirstChild("Rock") and v.Name:find("Rock")) then
                        currentRockPart = v:IsA("BasePart") and v or v:FindFirstChildOfClass("BasePart")
                        break
                     end
                  end
               end
      ...
      if hrp then
         if Flags.AntiKnockback then
            for _, v in pairs(hrp:GetChildren()) do
               if v:IsA("BodyVelocity") or v:IsA("LinearVelocity") or v:IsA("VectorForce") or v:IsA("BodyThrust") then
                  v:Destroy()
               end
            end
...
CreateToggle(RockPg, "Авто-Удар по камню", function(v)
   Flags.AirRock = v
   if not v and currentRockPart then
