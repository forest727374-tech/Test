-- // KECUYA HUB (ULTIMATE FIX: ALL ROCKS + FLY & ANTI-AFK TIMER)
...
-- Аниме графические ресурсы
local ANIME_GIRL_BG = "rbxassetid://11402212270"
local ANIME_GIRL_ICON = "rbxassetid://11402220022"
...
-- ==========================================
-- 1. СИСТЕМА ИНТЕРФЕЙСА (UI & ANTI-AFK COUNTER)
-- ==========================================
...
-- Логика Anti-AFK Таймера
local afkTimeCounter = 0
...
task.spawn(function()
    ...
    while task.wait(1) do
        ...
        if hrp then
            if lastHrpPos and (hrp.Position - lastHrpPos).Magnitude < 0.5 then
                afkTimeCounter = afkTimeCounter + 1
            else
                afkTimeCounter = 0
            end
        ...
        AntiAfkLabel.Text = "Anti-AFK (" .. tostring(afkTimeCounter) .. "s)"
    end
end)

-- Кнопки полета для мобильных
...
-- ==========================================
-- 2. ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ БОЯ И ВЗАИМОДЕЙСТВИЯ
-- ==========================================
...
-- ==========================================
-- 3. ГЛУБОКИЙ ПОИСК ВСЕХ КАМНЕЙ (ФИКС ДЛЯ ВСЕХ ТИПОВ)
-- ==========================================
local function GetRockPart(rockName)
    ...
    local searchTerms = {rockName:lower()}
    if rockName == "Tiny Rock" then table.insert(searchTerms, "tiny") end
    if rockName == "Medium Rock" then table.insert(searchTerms, "medium") end
    if rockName == "Large Rock" then table.insert(searchTerms, "large") end
    if rockName == "Huge Rock" then table.insert(searchTerms, "huge") end
    if rockName == "Legends Rock" then table.insert(searchTerms, "legends") end
    if rockName == "Jungle Rock" then table.insert(searchTerms, "jungle") end
    if rockName == "Muscle King Rock" then table.insert(searchTerms, "king") end

    for _, obj in pairs(Workspace:GetDescendants()) do
        if obj:IsA("BasePart") or obj:IsA("Model") then
            local nameLower = obj.Name:lower()
            for _, term in pairs(searchTerms) do
                if nameLower:find(term) then
    ...
    while true do
        if Flags.AirRock then
            ...
            if hrp then
                ...
                if rPart then
                    hrp.CFrame = rPart.CFrame * CFrame.new(0, rPart.Size.Y / 2 + 3.5, 0)
...
RunService.RenderStepped:Connect(function()
    ...
    if Flags.Fly and hrp and hum then
        ...
        if flyVel.Magnitude > 0 then
            hrp.AssemblyLinearVelocity = flyVel.Unit * FlySpeed
        else
            hrp.AssemblyLinearVelocity = Vector3.zero
        end
    else
...
-- РАСТЯГИВАНИЕ ХИТБОКСОВ И АТАКА
...
-- ==========================================
-- 6. ОБРАБОТЧИКИ АВТОРИЗАЦИИ И МЕНЮ
-- ==========================================
