-- // KECUYA HUB v3.0 (STABLE ENGINE & ADVANCED ANIME UI)
-- // Author: @kecuya | Key: Guest666
-- // Fully refactored with Modern Luau Spatial Queries & Physics Controls
...
local HitboxBoxSize = Vector3.new(4, 4, 4) -- Зона удара возле руки
...
-- ==========================================
-- 0. БЕЗОПАСНОСТЬ И АВТО-РЕКОННЕКТ
-- ==========================================
    ...
    while true do
        if Flags.AutoBoostEggs then
            pcall(function()
                ...
                if rEvents then
                    local openEgg = rEvents:FindFirstChild("openEggRemote") or rEvents:FindFirstChild("eggRemote")
                    if openEgg then
                        openEgg:InvokeServer("openEgg", "2x Boost Egg")
                        openEgg:InvokeServer("useBoost", "2x Strength")
                    end
                end
            end)
        end
...
-- ==========================================
-- 1. СИСТЕМА HITBOX Возле РУКИ (Spatial Query)
-- ==========================================

local function GetTargetInHandHitbox()
    local char = GetCharacter()
    if not char then return nil end
    ...
    -- Позиционирование области детектирования непосредственно перед кулаком
    local overlapParams = OverlapParams.new()
    overlapParams.FilterType = Enum.RaycastFilterType.Exclude
    overlapParams.FilterDescendantsInstances = {char}
    overlapParams.MaxParts = 10

    local hitboxCFrame = hand.CFrame * CFrame.new(0, 0, -2)
    local partsInBox = Workspace:GetPartBoundsInBox(hitboxCFrame, HitboxBoxSize, overlapParams)
    ...
    for _, part in ipairs(partsInBox) do
        ...
        if parentModel then
            ...
            if hum and targetHrp and hum.Health > 0 and not parentModel:FindFirstChildOfClass("ForceField") then
                local targetPlayer = Players:GetPlayerFromCharacter(parentModel)
                if targetPlayer then
                    local isFriend = false
                    pcall(function() isFriend = LocalPlayer:IsFriendsWith(targetPlayer.UserId) end)
...
local function GetTargetRock()
    ...
    for _, v in ipairs(Workspace:GetChildren()) do
        if v.Name:lower():gsub("%s+", ""):find(targetName, 1, true) or targetName:find(v.Name:lower():gsub("%s+", ""), 1, true) then
            if v:IsA("BasePart") then
                return v
            elseif v:IsA("Model") then
                local mainPart = v.PrimaryPart or v:FindFirstChild("Rock") or v:FindFirstChildWhichIsA("BasePart")
    ...
    while true do
        if Flags.AirRock then
            ...
            if char and rockPart then
                ...
                hrp.CFrame = CFrame.new(rockPos.Position + Vector3.new(0, (rockPart.Size.Y / 2) + 1.5, 0)) * CFrame.Angles(math.rad(-90), 0, 0)
...
-- ==========================================
-- 4. ОПТИМИЗИРОВАННЫЕ FLY И AUTO POSITION
-- ==========================================

local function ToggleFlyState(enabled)
    Flags.Fly = enabled
    MobileFlyFrame.Visible = enabled
...
RunService.RenderStepped:Connect(function()
    ...
    if Flags.AutoPosition and SavedPosition and not Flags.Fly then
...
-- ==========================================
-- 5. ТАРГЕТНАЯ АТАКА И KARMA FARMING
-- ==========================================

task.spawn(function()
    while true do
        if Flags.AutoKillAir or Flags.FarmGoodKarma or Flags.FarmEvilKarma or Flags.TargetKill then
            -- Проверка наличия врагов непосредственно в пространственном хитбоксе руки
            local handTarget = GetTargetInHandHitbox()
            if handTarget then
                ExecuteAttack(handTarget)
            else
                -- Поиск по глобальным игрокам/NPC при включенных режимах авто-килла
